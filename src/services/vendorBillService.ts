...
export const vendorBillService = {
  ...
  // An approver's veto: rejects the vendor bill while it awaits approval (a reason
  ...
  reject: (uuid: string, reason: string): Promise<VendorBill> =>
    tenantClient
      .post<{ success: boolean; vendorBill: VendorBill }>(`${BASE}/${uuid}/reject`, { reason })
      .then((r) => r.data.vendorBill),
