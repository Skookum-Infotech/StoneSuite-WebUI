...
import type { PipelineSegment } from '../mockData';
...
export function PipelineDonut({ segments }: { segments: PipelineSegment[] }) {
  ...
  const prospect = segments.find((s) => s.id === 'prospect');
  const customer = segments.find((s) => s.id === 'customer');
  const closeRate = prospect && customer && prospect.count > 0
    ? Math.round((customer.count / prospect.count) * 100)
    : 0;
