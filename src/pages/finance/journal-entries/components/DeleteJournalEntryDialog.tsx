...
// Mirrors DeleteItemReceiptDialog. The backend only accepts delete at Draft
// (cashtransfer/store_update.go SoftDelete, spec AD-9); callers gate the
// trigger on JE_DELETABLE_STATUSES so a 409 here should never actually
// happen, but the error still surfaces if it does.
export function DeleteJournalEntryDialog({ journalEntryId, label, onDeleted }: {
  ...
  const del = useMutation({
    mutationFn: () => journalEntryService.deleteJournalEntry(journalEntryId),
    onSuccess: () => {
      setOpen(false);
      onDeleted();
    },
  ...
  return (
    <>
      <DangerZoneAction
        ...
        buttonLabel="Delete journal entry"
      ...
      {open && createPortal(
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="delete-je-dialog-title"
        ...
        >
          <div ref={panelRef} tabIndex={-1} className="mx-4 w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl outline-none">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <h3 id="delete-je-dialog-title" className="text-sm font-bold text-stone-900">
            ...
            {del.error && (
              <p className="mb-3 text-xs text-destructive">
                {apiErrorMessage(del.error, 'Failed to delete journal entry.')}
              </p>
            )}
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {del.isPending ? 'Deleting…' : 'Delete journal entry'}
              </button>
            </div>
          </div>
        </div>,
