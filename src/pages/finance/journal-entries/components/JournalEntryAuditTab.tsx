...
// Mirrors ItemReceiptAuditTab, reading from journalEntryService.getAudit
// (/api/tenant/finance/cash-transfers/{uuid}/audit).
export function JournalEntryAuditTab({ journalEntryId }: { journalEntryId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['journal-entry-audit', journalEntryId],
    queryFn: () => journalEntryService.getAudit(journalEntryId!),
    enabled: Boolean(journalEntryId),
  });

  if (!journalEntryId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the journal entry.</p>;
  }
