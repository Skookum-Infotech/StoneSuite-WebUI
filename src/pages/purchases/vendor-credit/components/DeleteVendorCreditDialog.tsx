...
// application still references the credit (409); callers hide the trigger
// when applications exist, but the error still surfaces if it slips through.
export function DeleteVendorCreditDialog({ vendorCreditId, label, onDeleted }: {
  ...
  const del = useMutation({
    mutationFn: () => vendorCreditService.deleteVendorCredit(vendorCreditId),
    onSuccess: () => {
      setOpen(false);
      onDeleted();
    },
    ...
    <>
      ...
      {open && createPortal(
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="delete-vc-dialog-title"
        ...
        >
          <div ref={panelRef} tabIndex={-1} className="mx-4 w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl outline-none">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <h3 id="delete-vc-dialog-title" className="text-sm font-bold text-stone-900">
                  Delete vendor credit?
                </h3>
            ...
            {del.error && (
              <p className="mb-3 text-xs text-destructive">
                {apiErrorMessage(del.error, 'Failed to delete vendor credit.')}
              </p>
            )}
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {del.isPending ? 'Deleting…' : 'Delete vendor credit'}
              </button>
            </div>
          </div>
        </div>,
