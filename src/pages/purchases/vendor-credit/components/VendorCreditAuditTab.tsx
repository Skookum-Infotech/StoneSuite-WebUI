...
// Mirrors VendorPaymentAuditTab, reading from vendorCreditService.getAudit
// (/api/tenant/vendor-credits/{uuid}/audit).
export function VendorCreditAuditTab({ vendorCreditId }: { vendorCreditId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['vendor-credit-audit', vendorCreditId],
    queryFn: () => vendorCreditService.getAudit(vendorCreditId!),
    enabled: Boolean(vendorCreditId),
  });

  if (!vendorCreditId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the vendor credit.</p>;
  }
