...
import {
  EDIT_FIELDS, fromVendorCredit, toUpdatePayload, VC_EDITABLE_STATUSES,
  vcStatusLabel, PAGE_TABS, type PageTab,
} from '@/lib/vendorCreditForm';

// Stable empty default so the unsaved-changes baseline doesn't see a brand new
// object every render before the credit loads.
...
export default function EditVendorCreditPage() {
  ...
  const { data: credit, isLoading, error: loadError } = useQuery({
  ...
  const mapped = useMemo(() => (credit ? fromVendorCredit(credit) : null), [credit]);
  const data = localData ?? mapped?.data ?? {};
  const customFieldValues = localCustomFields ?? mapped?.customFieldValues ?? EMPTY_CUSTOM;
  const isLocked = credit ? !VC_EDITABLE_STATUSES.has(credit.statusCode) : false;

  // Baseline against the loaded record, not the empty defaults, so simply
  // opening the page never counts as an edit. A locked credit is read-only —
  // nothing to lose.
  ...
  const save = useMutation({
    mutationFn: () => vendorCreditService.updateVendorCredit(id, toUpdatePayload(data, customFieldValues)),
    onSuccess: (updated) => {
      queryClient.setQueryData(['vendor-credit', id], updated);
      queryClient.invalidateQueries({ queryKey: ['vendor-credits'] });
      guard.markClean();
      navigate(`/purchases/vendor_credit/${id}`);
    },
  });

  if (isLoading) return <div className="p-6"><Spinner label="Loading vendor credit…" /></div>;
  if (loadError || !credit)
    return <div className="p-6"><ErrorNote>{apiErrorMessage(loadError, 'Failed to load vendor credit.')}</ErrorNote></div>;

  // The backend accepts PATCH only at DRFT (vendorcredit/store_update.go).
  // Saying so up front beats letting the user retype a form the server will
  // reject.
  if (!VC_EDITABLE_STATUSES.has(credit.statusCode)) {
    return (
      <div className="p-6">
        <ErrorNote>
          {`A ${vcStatusLabel(credit.statusCode).toLowerCase()} vendor credit can no longer be edited.`}
        </ErrorNote>
      </div>
    );
  }
    ...
    <div className="flex flex-col flex-1 min-h-0 bg-stone-50">
      ...
      <form onSubmit={(e) => { e.preventDefault(); save.mutate(); }} className="flex flex-col flex-1 min-h-0">
        <CrmPageHeader
          ...
          onBack={() => navigate(`/purchases/vendor_credit/${id}`)}
          icon={FilePlus}
        ...
        {save.error && (
          <div className="shrink-0 flex items-start gap-3 border-b border-red-200 bg-red-50 px-5 py-2.5">
            ...
            <p className="text-xs text-red-700">
              ...
              {apiErrorMessage(save.error, 'Failed to save vendor credit.')}
            </p>
          </div>
        )}

        <VendorCreditFormBody
          shell={{ activeTab, setActiveTab, vendorCreditId: id }}
          form={{
            fields: EDIT_FIELDS, data, set, lookups,
            customFieldValues, setCustomField,
          }}
          vendor={{ value: mapped?.vendor ?? credit.vendor, onChange: () => undefined, locked: true }}
        />

        <FormActionBar
          onCancel={() => navigate(`/purchases/vendor_credit/${id}`)}
