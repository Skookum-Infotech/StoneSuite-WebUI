...
export function DeletePurchaseOrderDialog({ purchaseOrderId, label, onDeleted }: {
  ...
  return (
    <>
      <DangerZoneAction
        ref={triggerRef}
        description="Delete this purchase order permanently."
        buttonLabel="Delete purchase order"
        ...
        >
          <div ref={panelRef} tabIndex={-1} className="mx-4 w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl outline-none">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <h3 id="delete-po-dialog-title" className="text-sm font-bold text-stone-900">
            ...
            {del.error && (
              <p className="mb-3 text-xs text-destructive">
                {apiErrorMessage(del.error, 'Failed to delete purchase order.')}
              </p>
            )}
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {del.isPending ? 'Deleting…' : 'Delete purchase order'}
              </button>
            </div>
          </div>
        </div>,
