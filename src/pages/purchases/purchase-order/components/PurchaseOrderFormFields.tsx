...
import type { PurchaseOrderFormField } from '@/lib/purchaseOrderForm';

// Renders one PurchaseOrderFormField — shared by the Add and Edit Purchase
// Order forms. Mirrors EstimateFormFields' EstimateField/EstimateSectionGrid
// (no checkbox variant — the Purchase Order form has no toggle fields).
export function PurchaseOrderField({ field, value, set, lookups, dependsOnValue }: {
  field: PurchaseOrderFormField;
  value: unknown;
  ...
  if (field.type === 'select') {
    // lookupKey fields (country/state/payment terms/currency/employee) source
    // their options from CrmLookups by numeric id, so the stored value is the
    // id (string) — matching the create payload's *Id fields — rather than a
    // display label.
...
export function PurchaseOrderSectionGrid({ fields, data, set, lookups, maxCols = 3 }: {
  fields: PurchaseOrderFormField[];
