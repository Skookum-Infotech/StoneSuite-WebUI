...
// Mirrors EstimateAuditTab, reading from purchaseOrderService.getAudit
// (/api/tenant/purchase-orders/{uuid}/audit).
export function PurchaseOrderAuditTab({ purchaseOrderId }: { purchaseOrderId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['purchase-order-audit', purchaseOrderId],
    queryFn: () => purchaseOrderService.getAudit(purchaseOrderId!),
    enabled: Boolean(purchaseOrderId),
  });

  if (!purchaseOrderId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the purchase order.</p>;
  }
