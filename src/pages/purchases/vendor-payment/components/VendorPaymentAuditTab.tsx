...
// (/api/tenant/vendor-payments/{uuid}/audit).
export function VendorPaymentAuditTab({ vendorPaymentId }: { vendorPaymentId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['vendor-payment-audit', vendorPaymentId],
    queryFn: () => vendorPaymentService.getAudit(vendorPaymentId!),
    enabled: Boolean(vendorPaymentId),
  });

  if (!vendorPaymentId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the vendor payment.</p>;
  }
