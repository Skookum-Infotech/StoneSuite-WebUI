...
// Mirrors PurchaseOrderAuditTab, reading from requisitionService.getAudit
// (/api/tenant/requisitions/{uuid}/audit).
export function RequisitionAuditTab({ requisitionId }: { requisitionId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['requisition-audit', requisitionId],
    queryFn: () => requisitionService.getAudit(requisitionId!),
    enabled: Boolean(requisitionId),
  });

  if (!requisitionId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the requisition.</p>;
  }
