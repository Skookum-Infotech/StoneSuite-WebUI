...
// Requisitions are a dedicated relational module, not a generic CRM/JSONB
// workflow record, so — like Purchase Order/Estimate/Invoice — this table
// talks to requisitionService (/api/tenant/requisitions*) directly rather than
// reusing CrmRecordTable/crmService. Mirrors PurchaseOrderTable's search/sort/
// cursor-pagination UX.

// Only fields in requisition/resolver.go's `sortableFields` are offered.
// `needed_by_date` is deliberately absent: it's nullable, which breaks the
// keyset cursor's comparison, so the backend excludes it from the sort
// whitelist and a request for it would 400.
type SortField = 'recordNumber' | 'estimatedTotal' | 'priority' | 'status';
type SortDir = 'asc' | 'desc';

const SORT_LABELS: Record<SortField, string> = {
  recordNumber: 'Requisition #',
...
export function RequisitionTable() {
  ...
  const [filters, setFilters] = useState<RequisitionFilterState>(EMPTY_FILTER_STATE);
  ...
  const req: RequisitionSearchRequest = {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['requisitions', req],
    queryFn: () => requisitionService.searchRequisitions(req),
  ...
  // Cursors are opaque — only ever pass back what the server returned.
  function goNext() {
  ...
  function applyFilters(next: RequisitionFilterState) {
    setFilters(next);
    setCursor('');
    setPrevCursors([]);
  }
  ...
  async function handleDownloadCsv() {
    ...
    try {
      await exportPagedCsv(
        (exportCursor) => requisitionService.searchRequisitions({ ...req, limit: EXPORT_PAGE_SIZE, cursor: exportCursor }),
        ['Requisition #', 'Requested By', 'Department', 'Status', 'Approval', 'Priority', 'Needed By', 'Suggested Vendor', 'Estimated Total'],
        (reqn) => [
          reqn.requisitionNumber ?? '',
          (reqn.requestedByEmployeeId ? employeeNames.get(String(reqn.requestedByEmployeeId)) : undefined) ?? '',
          reqn.department ?? '',
          reqn.status ?? '',
          APPROVAL_LABELS[reqn.approvalStatus] ?? '',
          priorityLabel(reqn.priority ?? ''),
          fmtCsvDate(reqn.neededByDate),
          reqn.vendor?.name ?? '',
          String(reqn.estimatedTotal ?? 0),
        ],
        'Requisition',
      );
    } catch (err) {
    ...
    <div ref={topRef} className="flex flex-col gap-3 scroll-mt-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative w-full sm:w-64">
          ...
          <input
            type="text"
            placeholder="Search requisition #, vendor, item…"
            value={term}
        ...
        {records.length > 0 && (
          <button
            ...
            aria-label={hasFilters ? 'Download filtered requisitions as CSV' : 'Download all requisitions as CSV'}
      ...
      {/* A 400 here is an invalid filter key, not a generic failure —
          apiErrorMessage surfaces the server's InvalidFilterError text. */}
      {isError && (
        <p className="text-xs text-red-500">{apiErrorMessage(error, 'Failed to load requisitions. Please try again.')}</p>
      )}
      {exportError && (
                    ...
                    <tr key={reqn.id} className="group hover:bg-accent/10 transition-colors duration-150">
                      <td className="px-4 py-3.5">
                        <button
                          ...
                          aria-label={`Open requisition ${reqn.requisitionNumber}`}
                          className="inline-flex items-center gap-1.5 font-mono text-xs font-semibold text-stone-900 hover:text-accent-foreground transition-colors"
                        >
                          {reqn.requisitionNumber || '—'}
                          {reqn.convertedPurchaseOrderId && (
                            <Link2 className="size-3 text-stone-400" aria-label="Converted to a purchase order" />
                          )}
                        </button>
                      </td>
                      ...
                      <td className="px-4 py-3.5">
                        {approvalLabel ? (
                          <span
                            ...
                            style={{ backgroundColor: `${APPROVAL_COLORS[reqn.approvalStatus]}18`, color: APPROVAL_COLORS[reqn.approvalStatus] }}
                          >
                      ...
                      {canEdit && (
                        <td className="px-4 py-3.5 text-right">
                          <button
                            ...
                            aria-label={`Edit requisition ${reqn.requisitionNumber}`}
                  ...
                  <td colSpan={9 + (canEdit ? 1 : 0)} className="py-16 text-center">
                    {!hasFilters ? (
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm font-semibold text-stone-700">No requisitions raised yet.</p>
                        <p className="text-xs text-stone-400">Create your first requisition to get started.</p>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-3">
                        <div className="rounded-2xl bg-stone-100 p-4">
                        ...
                        <p className="text-sm font-semibold text-stone-700">No requisitions match the current search.</p>
