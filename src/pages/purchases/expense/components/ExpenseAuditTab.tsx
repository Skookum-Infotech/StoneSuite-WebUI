...
// Mirrors RequisitionAuditTab, reading from expenseService.getAudit
// (/api/tenant/expenses/{uuid}/audit).
export function ExpenseAuditTab({ expenseId }: { expenseId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['expense-audit', expenseId],
    queryFn: () => expenseService.getAudit(expenseId!),
    enabled: Boolean(expenseId),
  });

  if (!expenseId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the expense claim.</p>;
  }
