...
export default function ExpenseListPage() {
  ...
  const canCreate = isLoading || hasPermission('expense', 'create');
      ...
      <div className="p-4 sm:p-6 3xl:p-10 4xl:p-14 flex-1 flex flex-col min-h-0">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-stone-900">Expenses</h1>
              <p className="text-sm text-stone-500">Submit and approve employee expense claims.</p>
            </div>
          </div>
          {canCreate && (
            <button
              onClick={() => navigate('/purchases/expense/new')}
              aria-label="Create a new expense claim"
