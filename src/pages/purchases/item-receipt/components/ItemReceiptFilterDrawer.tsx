...
// Filter drawer for the Item Receipt list — mirrors PurchaseOrderFilterDrawer
// (same "mount only while open" pattern so `draft` re-initializes from
// `value` with no sync effect needed).
export function ItemReceiptFilterDrawer({ onClose, value, onApply }: {
  onClose: () => void;
  value: ItemReceiptFilterState;
  onApply: (next: ItemReceiptFilterState) => void;
}) {
  const [draft, setDraft] = useState<ItemReceiptFilterState>(value);
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="ir-filter-drawer-title"
      ...
      <div ref={contentRef} tabIndex={-1} className="flex h-full w-full max-w-sm flex-col bg-white shadow-2xl outline-none">
        <div className="flex items-center justify-between border-b border-stone-200 px-4 py-3.5 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 id="ir-filter-drawer-title" className="text-sm font-bold text-stone-900">Filters</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto modal-scrollbar px-4 py-4 space-y-5">
          <FilterField label="Receipt #">
            <input type="text" value={draft.recordNumber} onChange={(e) => set('recordNumber', e.target.value)} placeholder="Contains…" className={fieldCls} aria-label="Receipt #" />
          </FilterField>
          <FilterField label="PO #">
            <input type="text" value={draft.purchaseOrderNumber} onChange={(e) => set('purchaseOrderNumber', e.target.value)} placeholder="Contains…" className={fieldCls} aria-label="PO #" />
          </FilterField>
          ...
          <FilterField label="Packing Slip #">
            <input type="text" value={draft.packingSlip} onChange={(e) => set('packingSlip', e.target.value)} placeholder="Contains…" className={fieldCls} aria-label="Packing Slip #" />
          </FilterField>
          <FilterField label="Carrier">
            <input type="text" value={draft.carrier} onChange={(e) => set('carrier', e.target.value)} placeholder="Contains…" className={fieldCls} aria-label="Carrier" />
          </FilterField>
          <FilterField label="Tracking #">
            <input type="text" value={draft.trackingNumber} onChange={(e) => set('trackingNumber', e.target.value)} placeholder="Contains…" className={fieldCls} aria-label="Tracking #" />
          </FilterField>

          <FilterField label="Receipt Date">
            <div className="grid grid-cols-2 gap-2">
              <input type="date" value={draft.receiptDateFrom} onChange={(e) => set('receiptDateFrom', e.target.value)} className={fieldCls} aria-label="Receipt date from" />
              <input type="date" value={draft.receiptDateTo} onChange={(e) => set('receiptDateTo', e.target.value)} className={fieldCls} aria-label="Receipt date to" />
            </div>
          </FilterField>
