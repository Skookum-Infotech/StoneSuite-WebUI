...
// Mirrors PurchaseOrderAuditTab, reading from itemReceiptService.getAudit
// (/api/tenant/item-receipts/{uuid}/audit).
export function ItemReceiptAuditTab({ itemReceiptId }: { itemReceiptId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['item-receipt-audit', itemReceiptId],
    queryFn: () => itemReceiptService.getAudit(itemReceiptId!),
    enabled: Boolean(itemReceiptId),
  });

  if (!itemReceiptId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the item receipt.</p>;
  }
