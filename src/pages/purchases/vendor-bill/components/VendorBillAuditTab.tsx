...
// Mirrors PurchaseOrderAuditTab, reading from vendorBillService.getAudit
// (/api/tenant/vendor-bills/{uuid}/audit).
export function VendorBillAuditTab({ vendorBillId }: { vendorBillId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['vendor-bill-audit', vendorBillId],
    queryFn: () => vendorBillService.getAudit(vendorBillId!),
    enabled: Boolean(vendorBillId),
  });

  if (!vendorBillId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the vendor bill.</p>;
  }
