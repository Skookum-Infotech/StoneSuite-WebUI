...
function buildInitialSelected(role: Role, modules: PermModule[]): Record<string, RowSel> {
  ...
  for (const mod of modules) {
    for (const row of mod.rows) {
      ...
      if (perms.length > 0) {
        init[row.id] = { actions: perms.map((p) => p.action), scope: perms[0].scope };
      }
    }
  }
  return init;
...
export default function EditRolePage(): React.JSX.Element {
  ...
  if (rolesQ.isLoading || catalogQ.isLoading) {
    return <div className="flex flex-1 items-center justify-center p-12"><Spinner label="Loading…" /></div>;
  }
  ...
  return (
    <EditRoleInner
      ...
      actionsByResource={
        Object.fromEntries(
          (catalogQ.data?.permissions ?? []).reduce<Map<string, string[]>>((m, p) => {
...
function EditRoleInner({
  role, modules, actionsByResource, scopes,
}: {
  role: Role;
  modules: PermModule[];
  ...
  function toggleAction(rowId: string, action: string): void {
    setSelected((prev) => {
      ...
      if (!next.length) { const r = { ...prev }; delete r[rowId]; return r; }
  ...
  function toggleRow(row: ResourceRow): void {
    ...
    setSelected((prev) => {
      ...
      if (allSel) { const r = { ...prev }; delete r[row.id]; return r; }
      return { ...prev, [row.id]: { actions: avail, scope: prev[row.id]?.scope ?? 'all' } };
    });
  }
  ...
  function toggleColumn(mod: PermModule, action: string): void {
    ...
    setSelected((prev) => {
      ...
      for (const r of eligible) {
        ...
        const upd = allHave ? cur.filter((a) => a !== action) : [...new Set([...cur, action])];
  ...
  function getModuleChecked(mod: PermModule): boolean | 'indeterminate' {
    const eligible = mod.rows.filter((r) => getAvailable(r.resource).length > 0);
    if (!eligible.length) return false;
    const allSel = eligible.every((r) =>
      getAvailable(r.resource).every((a) => selected[r.id]?.actions.includes(a)),
    );
    ...
    setSelected((prev) => {
      ...
      if (checked === true) {
        ...
        for (const r of mod.rows) {
          ...
          if (avail.length) next[r.id] = { actions: avail, scope: prev[r.id]?.scope ?? 'all' };
        }
      }
      return next;
    });
  ...
  const update = useMutation({
    mutationFn: () => {
      ...
      return rbacService.updateRole(role.id, name.trim(), description.trim(), permissions);
    },
    onSuccess: () => {
    ...
    <div className="flex flex-1 flex-col min-h-0 bg-stone-50">
            ...
            <aside className="lg:sticky lg:top-6 lg:self-start">
              <div className="rounded-xl border border-stone-200 bg-white">
                <div className="border-b border-stone-100 px-4 py-3">
                  <h2 className="text-xs font-bold uppercase tracking-widest text-stone-500">Role Details</h2>
                </div>
                <div className="space-y-4 p-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="rname" className="text-xs font-medium text-stone-600">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-medium text-stone-600">Key</Label>
                    <div className="flex h-9 items-center rounded-md border border-stone-100 bg-stone-50 px-2.5">
                      <span className="text-xs font-mono text-stone-400">{role.key}</span>
                    </div>
                    <p className="text-2xs text-stone-400">Key cannot be changed after creation.</p>
                  </div>

                  <div className="space-y-1.5">
                    <Label htmlFor="rdesc" className="text-xs font-medium text-stone-600">Description</Label>
                    <textarea
                      id="rdesc"
                      value={description}
              ...
              <div className="rounded-xl border border-stone-200 bg-white overflow-hidden">
                ...
                <div>
                  {modules.map((mod, idx) => {
                    ...
                    const grantedCount = mod.rows.filter((r) => (selected[r.id]?.actions.length ?? 0) > 0).length;

                    const cols = ACTION_ORDER.filter((a) =>
                      mod.rows.some((r) => getAvailable(r.resource).includes(a)),
                    );

                    return (
                      <div key={mod.id} className={cn(idx > 0 && 'border-t border-stone-200')}>
                            ...
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="border-y border-stone-100 bg-white">
                                  ...
                                  {cols.map((action) => {
                                    const eligible = mod.rows.filter((r) => getAvailable(r.resource).includes(action));
                                    const allHave  = eligible.length > 0
                                      && eligible.every((r) => selected[r.id]?.actions.includes(action));
                                    return (
                                      <th key={action} className="w-20 py-2 text-center">
                                        <button
                                          type="button"
                              ...
                              <tbody>
                                {mod.rows.map((row) => {
                                  ...
                                  const rowAllSelected = available.length > 0
                                    && available.every((a) => checkedActions.includes(a));
                                  ...
                                  return (
                                    <tr
                                      ...
                                      className={cn(
                                        ...
                                        hasAny ? 'bg-brand/[0.02]' : 'hover:bg-stone-50/60',
                                      )}
                                    >
      ...
      <div className="border-t border-stone-200 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex w-full max-w-[1500px] items-center justify-between gap-3 px-6 py-3">
          <div className="min-w-0">
            {update.error && <ErrorNote>{apiErrorMessage(update.error)}</ErrorNote>}
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
