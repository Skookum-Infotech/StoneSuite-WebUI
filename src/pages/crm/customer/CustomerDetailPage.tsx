...
import { ArrowLeft, Building2, Pencil, Clock } from 'lucide-react';
...
const MOCK_HISTORY: HistoryEntry[] = [
  ...
  },
  {
    ...
    at: 'Yesterday',
    summary: 'Updated billing address and payment terms',
  },
  {
    id: '1',
    type: 'created',
              ...
              {entry.type === 'transition' && (
                <>
                  ...
                  <div className="flex items-center gap-1 mt-1.5">
                    ...
                    <span className="text-2xs bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded-md font-medium">{entry.toState}</span>
                  </div>
                </>
              )}
              {entry.type === 'edit' && (
      ...
      <div className="flex flex-col gap-0.5">
        ...
        <span
          className={cn(
            ...
            checked ? 'bg-emerald-50 text-emerald-700' : 'bg-stone-100 text-stone-400',
          )}
        >
          <span className={cn('h-1.5 w-1.5 rounded-full', checked ? 'bg-emerald-500' : 'bg-stone-300')} />
          {checked ? 'Yes' : 'No'}
        </span>
      </div>
    );
  ...
  if (field.type === 'textarea') {
    return (
      <div className="sm:col-span-2 flex flex-col gap-0.5">
        ...
        <span className="text-sm font-medium text-stone-800 leading-snug whitespace-pre-wrap break-words">
          {String(value ?? '') || <span className="text-stone-300 font-normal text-xs">—</span>}
        </span>
      </div>
    );
