...
export default function LoginPage() {
  ...
  function redirectToSso(provider: SAMLProvider, opts: { tenantId?: string; tenantSlug?: string }) {
    ...
    sessionStorage.setItem(SAML_PENDING_PROVIDER_KEY, provider)
    window.location.href = samlAuthService.initiateUrl(provider, { ...opts, returnTo })
  }

  // A distributed sign-in link (?tenant_id=...&sso=entra, generated by the
  // SAML setup page's copyable link) redirects immediately -- no extra click.
  useEffect(() => {
    const tenantId = searchParams.get('tenant_id') ?? undefined
    const tenantSlug = searchParams.get('workspace') ?? undefined
    const sso = searchParams.get('sso')
    if ((tenantId || tenantSlug) && (sso === 'entra' || sso === 'cognito')) {
      redirectToSso(sso, { tenantId, tenantSlug })
    }
    // Only ever run once, on the link that brought the user here.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  function startSso(provider: SAMLProvider) {
    const tenantId = searchParams.get('tenant_id') ?? undefined
    const tenantSlug = searchParams.get('workspace') ?? undefined
