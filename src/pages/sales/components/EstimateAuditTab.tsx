...
// Mirrors InvoiceAuditTab, but reads from estimateService.getAudit
// (/api/tenant/estimates/{uuid}/audit) rather than the generic CRM audit
// endpoint or the Invoice one.
export function EstimateAuditTab({ estimateId }: { estimateId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['estimate-audit', estimateId],
    queryFn: () => estimateService.getAudit(estimateId!),
    enabled: Boolean(estimateId),
  });

  if (!estimateId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the estimate.</p>;
  }
