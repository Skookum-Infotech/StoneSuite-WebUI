...
// Mirrors InvoiceAuditTab, but reads from creditMemoService.getAudit
// (/api/tenant/credit-memos/{uuid}/audit).
export function CreditMemoAuditTab({ creditMemoId }: { creditMemoId?: string }) {
  const { data: entries = [], isLoading, error } = useQuery({
    queryKey: ['credit-memo-audit', creditMemoId],
    queryFn: () => creditMemoService.getAudit(creditMemoId!),
    enabled: Boolean(creditMemoId),
  });

  if (!creditMemoId) {
    return <p className="py-12 text-center text-sm text-stone-400">Audit trail will be available after saving the credit memo.</p>;
  }
