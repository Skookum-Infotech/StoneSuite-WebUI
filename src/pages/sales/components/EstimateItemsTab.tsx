...
// Mirrors InvoiceItemsTab — an estimate line has no free-text sku/units/tax%
...
export function EstimateItemsTab({ items, onUpdate, headerTaxPercent }: {
  ...
  const [draft, setDraft] = useState<Omit<EstimateLineItem, 'id' | 'lineNo'>>(EMPTY_LINE_ITEM);
  const [editId, setEditId] = useState<string | null>(null);
  const [isAdding, setIsAdding] = useState(false);

  const recalc = (next: Omit<EstimateLineItem, 'id' | 'lineNo'>) => {
  ...
  const startEdit = (row: EstimateLineItem) => {
    setEditId(row.id);
