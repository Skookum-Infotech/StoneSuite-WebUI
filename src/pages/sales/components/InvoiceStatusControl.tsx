...
import { INVOICE_STATUS_CODES, INVOICE_ALLOWED_TRANSITIONS, INVOICE_STATUS_COLORS, needsApproval, invoiceTransitionLabel } from '@/lib/invoiceForm';
import { StatusSelect } from './StatusSelect';
import type { Invoice } from '@/types/invoice';

// Status select for the Invoice Edit/Detail pages. Legal moves mirror the
// backend invoice/transitions.go (spec §7), except that the record's own
...
// Every move needs the single invoice:transition permission. A move is
// additionally blocked client-side while the invoice is gated on approval
// (AD-8) -- the backend would 409 with ErrApprovalRequired anyway, this
// just explains why up front instead of after a failed save. Use the
...
export function InvoiceStatusControl({ invoice, onChange, disabled, variant }: {
  invoice: Pick<Invoice, 'statusCode' | 'approvalStatus' | 'nextStatusCodes'> & { gated?: boolean };
  ...
  return (
    <StatusSelect
      ...
      nextCodes={invoice.nextStatusCodes}
      guard={guard}
      variant={variant}
      colorFor={(s) => INVOICE_STATUS_COLORS[s.label] ?? '#a8a29e'}
      labelFor={(s, fromCode) => invoiceTransitionLabel(fromCode, s.code)}
    />
  );
}
