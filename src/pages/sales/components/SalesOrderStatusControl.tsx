...
import { SO_STATUS_CODES } from '@/lib/salesOrderForm';

// Status select for the Sales Order Edit page, mirroring StatusDropdown's
// look. Unlike CRM's generic v1 workflow, Sales Order has a small, fixed
// state machine (spec §8) with no admin-configurable states, so this is a
// static list rather than a fetched one. The backend (ValidateTransition) is
// the source of truth for which moves are legal from the current status — an
...
export function SalesOrderStatusControl({ value, onChange, disabled }: {
  ...
  const selected = SO_STATUS_CODES.find((s) => s.code === value);
