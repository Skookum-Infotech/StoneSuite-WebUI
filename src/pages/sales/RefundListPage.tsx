...
export default function RefundListPage() {
  ...
  // merged-login design) but never creates a refund — the backend has no
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-accent ring-1 ring-accent-foreground/10 shrink-0">
              <Undo2 className="size-5 text-accent-foreground" aria-hidden="true" />
            </div>
            <div>
              <h1 className="text-xl font-bold tracking-tight text-stone-900">Refunds</h1>
              <p className="text-sm text-stone-500">Money returned to customers.</p>
            </div>
          </div>
          {!isCustomer && (
            <button
              onClick={() => navigate('/sales/refund/new')}
