 ...
 *  for PAPV, the backend (estimate/store_transition.go) auto-skips straight
 *  to Approved. Every other pair keeps its plain status label. */
export function estimateTransitionLabel(from: string, to: string): string {
  if (from === 'DRFT' && to === 'PAPV') return 'Submit for Approval';
  return ESTIMATE_STATUS_CODES.find((s) => s.code === to)?.label ?? to;
}
...
export function fromEstimate(estimate: Estimate): {
  ...
  return {
    ...
    customFieldValues: estimate.customFields ?? {},
  };
}
