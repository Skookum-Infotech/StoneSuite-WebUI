...
export const apiClient = axios.create({
  baseURL: import.meta.env.VITE_API_BASE_URL || 'https://dev-stonesuite-api.fly.dev/api',
    ...
    refreshPromise = (async () => {
      try {
        const res = await axios.post<{ success: boolean; token?: string; expiresAt?: number }>(
          `${import.meta.env.VITE_API_BASE_URL || 'https://dev-stonesuite-api.fly.dev/api'}/auth/refresh`,
          {},
          { withCredentials: true },
        );
...
function forceLogout(): void {
  ...
  axios
    .post(
      `${import.meta.env.VITE_API_BASE_URL || 'https://dev-stonesuite-api.fly.dev/api'}/auth/logout`,
      {},
      { withCredentials: true },
    )
    .catch(() => undefined);
